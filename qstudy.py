...
class CircuitSlicer:
    def __init__(self, algo, nsims=1, options=None, common_factors=True, bloch=True, postproc=None, matrix_precision=10):
        self.algo = algo
        self.matrix_precision = matrix_precision
        if options is None:
            options = algo.get_options()
        self.labels = [inst.label for i, inst in enumerate(algo.get_circuit(options[0][1]))
            if inst.operation.name == "barrier"]
        self.labels.append("final")

        self.nsims = widgets.IntText(nsims, description="# of sims: ")
        self.option = widgets.Dropdown(options=options, description="Option: ")
        self.step = widgets.SelectionSlider(options=self.labels, description="Step: ")
        ...
        self.bloch = widgets.Output() if bloch else None
        ...
        tab = widgets.Tab()
        tab.children = [main, self.stats]
        tab.set_title(0, "Main")
        tab.set_title(1, "Stats")
        if bloch:
            tab.children += (self.bloch,)
    ...
    def instrument_circuit(self, c):
        ...
        for i, inst in enumerate(c.data):
            if inst.operation.name == "barrier":
                l = inst.operation.label
                new.save_statevector(label=l)
                new.save_density_matrix(live, label=l + ":dm")
